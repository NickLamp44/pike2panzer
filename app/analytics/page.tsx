...
function calculateStatistics(): Statistics {
  ...
  eras.forEach((era) => {
    totalEras++;
    const conflictCount = 3; // Approximate based on structure
    eraBreakdown.push({ era: era.title, conflicts: conflictCount });
    totalConflicts += conflictCount;

    // Extract year from period
    if (era.period) {
      const years = era.period.match(/\d{3,4}/g);
      if (years && years.length > 0) {
        const firstYear = years[0];
        const lastYear = years[years.length - 1];
        
        if (firstYear < oldestDate) oldestDate = firstYear;
        if (lastYear > newestDate) newestDate = lastYear;
      }
    }

    // Rough estimates based on typical era data
    totalCommanders += 5;
    totalBattles += 10;
    totalWeapons += 8;
    totalTactics += 6;
  ...
  return {
    ...
    totalWeaponTechs,
    eraBreakdown,
    dateRange: { oldest: oldestDate === '9999' ? '1200' : oldestDate, newest: newestDate === '0000' ? '1945' : newestDate },
  };
}
